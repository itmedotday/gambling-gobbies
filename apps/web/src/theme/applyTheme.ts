...
import type { ThemeStyle, UiSkin } from '@/stores/settingsStore';
...
/** Single entry point for root appearance: palette, skin class, color-scheme, motion. */
export function applyThemeAppearance({ mode, style, skin, reducedMotion }: ThemeAppearance): void {
  if (typeof document === 'undefined') return;
  const root = document.documentElement;

  applyPaletteAppearance(mode, style as PaletteStyle);
  root.classList.toggle('dark', mode === 'dark');
  root.classList.toggle('skin-shadcn', skin === 'shadcn');
  root.classList.toggle('skin-eightbit', skin === 'eightbit');
