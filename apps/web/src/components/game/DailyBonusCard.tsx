import { useState } from 'react';
import { toast } from 'sonner';
import { Gift } from 'lucide-react';
import { Button } from '@/components/ui/8bit/button';
import { Card, CardContent } from '@/components/ui/8bit/card';
import { DAILY_BONUS_AMOUNT } from '@gobbies/shared';
import { useWalletStore } from '@/stores/walletStore';

export function DailyBonusCard() {
  const claimDailyBonus = useWalletStore((s) => s.claimDailyBonus);
  const canClaimDailyBonus = useWalletStore((s) => s.canClaimDailyBonus);
  // canClaim depends on wall-clock time, so track claims locally to re-render.
  const [, setClaimCount] = useState(0);
  const claimable = canClaimDailyBonus();

  const handleClaim = () => {
    if (claimDailyBonus()) {
      toast.success(`The goblins spot you ${DAILY_BONUS_AMOUNT} GG. Don't spend it all in one pit.`);
    } else {
      toast.error('Bonus already claimed. Come back later.');
    }
    setClaimCount((c) => c + 1);
  };

  return (
    <Card>
      <CardContent className="flex flex-col items-start justify-between gap-4 p-5 sm:flex-row sm:items-center">
        <div className="flex items-center gap-3">
          <Gift className="size-6 text-primary" aria-hidden />
          <div className="flex flex-col">
            <span className="retro text-[11px] text-primary">Daily bonus</span>
            <span className="text-sm text-muted-foreground">
              {DAILY_BONUS_AMOUNT} GG of goblin generosity, every 20 hours.
            </span>
          </div>
        </div>
        <Button onClick={handleClaim} disabled={!claimable} data-testid="daily-bonus-claim">
          {claimable ? 'Claim' : 'Claimed'}
        </Button>
      </CardContent>
    </Card>
  );
}
